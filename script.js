/* =====================================================================
   FreedomNumber — script.js (Start+Tenure edition, v1.3)
   - Matches your current HTML layout (Start age + Tenure)
   - Editable rows + Add/Remove (Regular & Planned)
   - Live KPIs (India-first) + early depletion warning
...
(function () {
  ...
  try { console.log("[FreedomNumber] script loaded v1.3 (tenure/start)"); } catch (e) {}
  ...
  function cleanNum(str) { return parseFloat(String(str).replace(/[₹,\s]/g, "")) || 0; }
  function escapeHtml(s) { return String(s).replace(/[&<>"']/g, function (m) { return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]; }); }
  function money(n) { return isNaN(n) ? "—" : ("₹ " + Math.round(n).toLocaleString("en-IN")); }
  function pct(x, d) { d = (typeof d === "number") ? d : 2; return isNaN(x) ? "—" : ((x*100).toFixed(d) + "%"); }
  ...
  function getInputs() {
    ...
    var i = {
      ...
      ret: num("ret"),               // expected annual return (after tax & fees) unless TER>0
    ...
    // gentle bounds
    i.currentAge = clamp(i.currentAge, 18, 100);
    i.freedomAge = clamp(i.freedomAge, 30, 80);
    i.lifeAge    = clamp(i.lifeAge, Math.max(i.freedomAge, 60), 100);
    i.ret        = clamp(i.ret, 0, 0.15);
    i.inflation  = clamp(i.inflation || 0.06, 0, 0.20);
    i.buffer     = clamp(i.buffer || 0.15, 0, 0.30);
    i.ter        = clamp(i.ter || 0, 0, 0.03);

    // If TER provided (>0), treat ret as gross and net it. Otherwise ret already net.
    if (i.ter > 0) i.ret = Math.max(0, i.ret - i.ter);

    return i;
  }

  // REGULAR table: Category | Amount | Growth | Tenure | Start
  // PLANNED table: Event | Event age | Amount today | Inflation
  function getTables() {
    var regs = qa("#regTable tbody tr").map(function (tr) {
      var td = tr.querySelectorAll("td");
      var nm = tr.querySelector("input[data-field='name']");
      var am = tr.querySelector("input[data-field='amount']");
      var gr = tr.querySelector("input[data-field='growth']");
      var te = tr.querySelector("input[data-field='tenure']");
      var st = tr.querySelector("input[data-field='start']");
      return {
        name:  nm ? nm.value : ((td[0] && td[0].textContent.trim()) || "Item"),
        amount: am ? (parseFloat(am.value)||0) : cleanNum((td[1] && td[1].textContent)||""),
        growth: gr ? (parseFloat(gr.value)||0) : (parseFloat((td[2] && td[2].textContent)||"0")||0),
        tenure: te ? (parseFloat(te.value)||0) : (parseFloat((td[3] && td[3].textContent)||"0")||0),
        start:  st ? (parseFloat(st.value)||0) : (parseFloat((td[4] && td[4].textContent)||"0")||0)
      };
    });
    ...
    var plans = qa("#planTable tbody tr").map(function (tr) {
      var td = tr.querySelectorAll("td");
      var nm = tr.querySelector("input[data-field='name']");
      var ev = tr.querySelector("input[data-field='event']");
      var am = tr.querySelector("input[data-field='amount']");
      var inf= tr.querySelector("input[data-field='infl']");
      return {
        name:  nm ? nm.value : ((td[0] && td[0].textContent.trim()) || "Event"),
        event: ev ? (parseFloat(ev.value)||0) : (parseFloat((td[1] && td[1].textContent)||"0")||0),
        amount: am ? (parseFloat(am.value)||0) : cleanNum((td[2] && td[2].textContent)||""),
        infl:  inf ? (parseFloat(inf.value)||0) : (parseFloat((td[3] && td[3].textContent)||"0")||0)
      };
    });
  ...
  function computeProjection(inputs, regs, plans) {
    ...
    for (var a = freedomAge; a <= lifeAge; a++) {
      // Regular active if start <= a < start+tenure
      var regSum = regs.reduce(function (s, r) {
        var active = (a >= r.start) && (a < r.start + r.tenure);
        if (!active) return s;
        var g = (typeof r.growth === "number") ? r.growth : inflation;
        var yrs = Math.max(0, a - r.start);
        return s + r.amount * Math.pow(1 + g, yrs);
      }, 0);

      // Planned events at age a (inflated from today/currentAge)
      ...
      // Effective return (stress: -2% real first 10 post-ret years)
      var rEff = ret;
  ...
  function computeAnnualToday(regs, plans, currentAge) {
    var regSum = regs.reduce(function (a, r) {
      ...
      if (!active) return a;
      var yrs = Math.max(0, currentAge - r.start);
      var g = (typeof r.growth === "number") ? r.growth : 0.06;
      return a + r.amount * Math.pow(1 + g, yrs);
    }, 0);
    var planToday = plans.reduce(function (a, p) { return a + (p.event === currentAge ? p.amount : 0); }, 0);
  ...
  function renderKPIs(proj, inputs, regs, plans) {
    ...
    if (!proj.length) {
      ...
      setText("kpiAnnual","—"); setText("kpi40x","—");
      return;
    }
    ...
    // early depletion
    var depAge = null;
    for (var i=0;i<proj.length;i++){ if (proj[i].endCorpus < 0){ depAge = proj[i].age; break; } }
    if (early) {
      if (depAge!==null) { early.textContent = "Warning: Corpus depletes at age "+depAge+". Consider higher SIP, later retirement, trimming spend, or revising return assumptions."; early.hidden = false; }
      else early.hidden = true;
    }
    ...
    var badge = $("kpiStatus"), banner = $("summaryBanner");
    ...
    var narrative =
      ...
      (end < -1 ? "Add about "+money(Math.abs(end))+" today (or increase SIP / delay retirement / trim spend)."
                : end > 1 ? "On current settings you retain a surplus of "+money(end)+" by age "+inputs.lifeAge+"."
  ...
  function exportCSV(proj) {
    ...
    proj.forEach(function (r) {
      lines.push([r.age,Math.round(r.reg),Math.round(r.planned),Math.round(r.total),Math.round(r.totalWithBuffer),Math.round(r.startCorpus),Math.round(r.ret),Math.round(r.endCorpus)].join(","));
    });
    var blob = new Blob([lines.join("\n")], {type:"text/csv;charset=utf-8"});
    var url = URL.createObjectURL(blob);
    var a = document.createElement("a");
    a.href = url; a.download = "freedomnumber_projection.csv";
  ...
  function openAudit(proj, regs, plans) {
    ...
    var close = $("auditClose");
    if (close) close.onclick = function(){ backdrop.style.display = "none"; };
    var go = $("auditGo");
    if (go) go.onclick = function(){ renderAuditForAge(num("auditAge"), proj, regs, plans); };
  }

  function renderAuditForAge(age, proj, regs, plans) {
    var row = null; for (var i=0;i<proj.length;i++){ if (proj[i].age===age){ row = proj[i]; break; } }
    var s=$("auditSummary"), ar=$("auditRegular"), ap=$("auditPlanned"), agg=$("auditAgg");
    if (!row) {
      if (s) s.textContent = "No data for age "+age+".";
      if (ar) ar.textContent = ""; if (ap) ap.textContent = ""; if (agg) agg.textContent = "";
      return;
    }
    if (s) s.textContent =
    ...
    if (ar) {
      ...
      regs.forEach(function (r) {
        if (age >= r.start && age < r.start + r.tenure) {
          var yrs = Math.max(0, age - r.start);
          var g = (typeof r.growth==="number") ? r.growth : 0;
          var val = r.amount * Math.pow(1 + g, yrs);
          linesR.push(r.name+": "+money(val)+"  (amt "+money(r.amount)+", g "+pct(g)+", start "+r.start+", tenure "+r.tenure+")");
        }
      });
    ...
    if (ap) {
      ...
      plans.forEach(function (p) {
        if (p.event === age) linesP.push(p.name + ": " + money(p.amount) + " (today) @ i " + pct(p.infl));
      });
  ...
  function saveState() {
    try {
      var inputs = getInputs(); var t = getTables();
  ...
  function applyInputs(i){
    ...
    if (on&&off){
      if (i.stressOn){ on.setAttribute("aria-pressed","true"); off.setAttribute("aria-pressed","false"); }
  ...
  function loadStateIfAny(){
    try{
      ...
      if (h){
        ...
        if (s && s.i && s.r && s.p){ applyInputs(s.i); renderTablesEditable(s.r, s.p); return true; }
      }
      var I=localStorage.getItem("inputs"), R=localStorage.getItem("regs"), P=localStorage.getItem("plans");
      if (I && R && P){ applyInputs(JSON.parse(I)); renderTablesEditable(JSON.parse(R), JSON.parse(P)); return true; }
    }catch(e){}
    return false;
  }
  ...
  function bindInputs(){
    ...
    [["life85",85],["life90",90],["life95",95]].forEach(function(pair){
      ...
      b.addEventListener("click", function(){
        var life=$("lifeAge"); if(life) life.value=pair[1];
    ...
    var on=$("stressOn"), off=$("stressOff");
    if (on && off){
      on.addEventListener("click", function(){ on.setAttribute("aria-pressed","true"); off.setAttribute("aria-pressed","false"); recompute(); });
      off.addEventListener("click", function(){ on.setAttribute("aria-pressed","false"); off.setAttribute("aria-pressed","true"); recompute(); });
    }
    ...
    if (addReg) addReg.addEventListener("click", function(){
      var t=getTables();
      var cAge=num("currentAge"), lAge=num("lifeAge");
      var tenure = Math.max(1, lAge - cAge);
      t.regs.push({ name:"New item", amount:60000, growth:0.06, tenure:tenure, start:cAge });
    ...
    if (addPlan) addPlan.addEventListener("click", function(){
      var fAge=num("freedomAge"); var t=getTables();
    ...
    // tools
    var exportBtn=$("exportBtn"); if(exportBtn) exportBtn.addEventListener("click", function(){ exportCSV(window.__proj||[]); });
    var shareBtn=$("shareBtn"); if(shareBtn) shareBtn.addEventListener("click", function(){
      var inputs=getInputs(), t=getTables(); var hash=encodeShareState(inputs, t.regs, t.plans);
      if (hash){
        var url = location.origin + location.pathname + "#" + hash;
        try { navigator.clipboard.writeText(url).then(function(){ alert("Shareable link copied to clipboard!"); }, function(){ prompt("Copy your shareable link:", url); }); }
        catch(e){ prompt("Copy your shareable link:", url); }
      }
    });
    var auditBtn=$("auditBtn"); if(auditBtn) auditBtn.addEventListener("click", function(){ var T=getTables(); openAudit(window.__proj||[], T.regs, T.plans); });

    // reset / prefill
    var resetBtn=$("resetBtn"); if(resetBtn) resetBtn.addEventListener("click", function(){ try{localStorage.clear();}catch(e){} location.reload(); });
    var loadIndicative=$("loadIndicative"); if(loadIndicative) loadIndicative.addEventListener("click", function(){ try{localStorage.clear();}catch(e){} location.reload(); });
  }

  function init(){
    try { console.log("[FreedomNumber] init start"); } catch(e){}
    bootstrapTablesFromDOM();     // convert default rows -> inputs
    loadStateIfAny();             // restore saved/hash state if present
    bindInputs();                 // attach listeners
